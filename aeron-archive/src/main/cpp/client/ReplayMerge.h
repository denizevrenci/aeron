...
constexpr std::int64_t REPLAY_MERGE_LIVE_ADD_THRESHOLD = LogBufferDescriptor::TERM_MIN_LENGTH / 4;
constexpr std::int64_t REPLAY_MERGE_REPLAY_REMOVE_THRESHOLD = 0;
...
public:
    ...
    {
        doWork();
        return !m_image ? 0 : m_image->poll(fragmentHandler, fragmentLimit);
    }
...
private:
    ...
    State m_state = State::AWAIT_INITIAL_RECORDING_POSITION;
